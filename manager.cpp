...
int manager :: questions_counter = 0;
void manager::LoadData() {
 ...
 file_q.open("questions file.txt" , ios::in);
  ...
  file_q >> Answer >> Answer >> Answer >> Answer >> Answer>> Answer;
  while(file_q.peek() != EOF){
    questions_counter++;
  file_q >> q_ID >> sender_id >> receiver_id >> qustion_text >> Answer>> anon;
...
//        }
  ...
  if(file_th.fail()){
    ...
    int pId;
  file_th >> Answer >> Answer >> Answer >> Answer >> Answer >> Answer;
  while(file_th.peek() != EOF){
    questions_counter++;
    file_th >> q_ID >> pId >> sender_id >> receiver_id >> qustion_text >> Answer;
...
void manager::SaveData() {
  ...
  file_q << "question_Id " << "sender_ID " << "receiver_ID " <<"question " << "Answer " << "anonymous";
  file_th << "QId " << "QPairentid " << "seID " << "recID " << "qtext " << "Answer";
  for(auto a : users){
